/**************************************************************************************************
  ...
  Revised:        $Date: 2011-08-01 09:51:47 -0700 (Mon, 01 Aug 2011) $
  Revision:       $Revision: 26995 $
    ...
    else //timer is running, so allow simulated changes
    {
      ...
      else
      {
        ...
        if(temperatureIntervalConfig == true) 
        {
          ...
          status = Thermometer_Indicate( gapConnHandle, &intervalIndication,  thermometerTaskId );
...
static void thermometerSendStoredMeas(void)
{
  ...
  if(thStoreStartIndex != thStoreIndex )
    {
     //send Measurement
     status  = Thermometer_Indicate( gapConnHandle, &thStoreMeas[thStoreStartIndex],  thermometerTaskId);
...
static void thermometerStoreIndications(attHandleValueInd_t* pInd)
{
    ...
    //store measurement
    osal_memcpy(&thStoreMeas[thStoreIndex],pInd, ATT_MTU_SIZE);
