 ...
 * \ref USBIRQ_EVENT_SUSPEND event. The USB device must then enter suspend mode within 7 ms (so within 10 ms of the end
 * of activity on the USB bus), where it draws no more than 2.5 mA averaged over 1 second.
 *
 ...
 *     7 ms. If the worst-case path through the main loop uses more than 7 ms, the code block can be
