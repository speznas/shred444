/**************************************************************************************************
  ...
  Revised:        $Date: 2011-06-30 13:29:52 -0700 (Thu, 30 Jun 2011) $
  Revision:       $Revision: 26533 $
...
void HCI_EXT_App_Init( uint8 task_id )
{
  ...
  HalLcdWriteString( "  TI  BLEv1.1", HAL_LCD_LINE_1 );
  HalLcdWriteString( "  HostTestApp", HAL_LCD_LINE_2 );
  #if defined(DEVICE_TYPE) && (DEVICE_TYPE & LL_DEV_TYPE_MASTER)
    HalLcdWriteString( "     Master", HAL_LCD_LINE_3 );
  #else
    HalLcdWriteString( "     Slave", HAL_LCD_LINE_3 );
  #endif
  ...
  *pRspDataLen = 0;
  ...
  switch( cmdID )
  {
    case HCI_EXT_UTIL_RESET:
      ...
      {
        VOID osal_start_timerEx( hciExtApp_TaskID, HCI_EXT_SOFT_RESET_EVENT, RESET_TIMEOUT );
      }
      break;
